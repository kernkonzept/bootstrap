...
scan_ram_size(unsigned long base_addr, unsigned long max_scan_size_mb)
{
  ...
  extern char _end[];
#ifdef IMAGE_MODE
  extern char _end_of_initial_bootstrap[];
  l4_addr_t lower_bound
    = l4_round_size(cxx::max((l4_addr_t)_end, (l4_addr_t)_end_of_initial_bootstrap), 20);
#else
  ...
  printf("  Scanning up to %lu MiB RAM, starting at offset %lu MiB\n",
...
setup_single_region_ram_memory_map()
{
  ...
  printf("  Memory size is %lu MiB%s (%08lx - %08lx)\n",
